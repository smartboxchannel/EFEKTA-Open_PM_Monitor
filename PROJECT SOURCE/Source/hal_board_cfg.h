...
// Configure USART0 in SPI mode (Alt. 2)
#define XNV_SPI_INIT() \
st( \
  /* Configure USART0 SPI Master */ \
  U0CSR = 0; \
  /* Set baud rate (115200 approx) */ \
  U0GCR |= 11; \
  U0BAUD = 216; \
  \
  /* SPI mode - MSB first */ \
  U0GCR |= BV(5); \
  \
  /* Alternate 2 for USART0 (P1.5 - MOSI, P1.4 - MISO, P1.3 - CLK) */ \
  PERCFG |= 0x01;  /* U0CFG = 0 (Alt. 2) */ \
  \
  /* Configure peripheral pins */ \
  P1SEL |= 0x38;   /* P1.5 (MOSI), P1.4 (MISO), P1.3 (CLK) */ \
  P1SEL &= ~0x04;  /* P1.2 (CS) as GPIO */ \
  /* Configure CS pin */ \
  P1 |= 0x04;      /* P1.2 (CS) high by default */ \
  P1DIR |= 0x04;   /* P1.2 (CS) as output */ \
  \
  /* Configure USART0 */ \
  P2SEL &= ~0x08;  /* PRI3P1 = 0 (USART0 has priority) */ \
  \
  /* Enable USART0 in SPI mode */ \
  U0CSR |= 0x40; \
)
